 ...
 *
 *        Version:  1.0
 *        Created:  03/26/2013 12:05:16 AM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
namespace http {
	namespace server {
		...
		std::vector<std::string> reply::to_buffers(){
		...
		void reply::stock_reply(reply::status_type status, std::ostream &out){
			out << status_strings::to_buffer(status) <<
				"content-type: text/html" <<
				misc_strings::crlf<<misc_strings::crlf <<
				stock_replies::to_string(status);
		}
		
		void reply::stock_reply(reply::status_type status, std::ostream &out, const std::string &extension){
			out << status_strings::to_buffer(status) <<
